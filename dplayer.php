...
<?php
    ...
    public static function dplayer_load($atts, $content, $tag) {
        ...
        $video = array(
            'url' => $atts['url'] ? ($atts['url']."?key=".md5("xiaofan666666".$_SERVER['REMOTE_ADDR'])) : '',
    ...
    public static function add_script() {
        if (!self::$add_script) {
            ...
            if ($current_time - $last_check > 86400 /* 86400 = 60 * 60 * 24 i.e 24hrs */) {
                //$response = wp_remote_get( 'www.meeboy.com/hi/wp-content/plugins/dplayer-for-wp/dplayerhttps://cdnjs.cat.net/ajax/libs/dplayer/package.json' );
                $response = wp_remote_get( '/wp-content/plugins/dplayer-for-wp/dplayer/package.json' );
            ...
            //wp_enqueue_style( 'dplayer', esc_url("https://cdn.jsdelivr.net/npm/p2p-dplayer@latest/dist/DPlayer.min.css"), false, $dplayer_version, false );
            wp_enqueue_style( 'dplayer', esc_url("/wp-content/plugins/dplayer-for-wp-p2p/dplayer/libs/DPlayer.min.css?$dplayer_versions"), false, $dplayer_version, false );
